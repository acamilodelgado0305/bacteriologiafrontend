...
const ModalCredenciales = ({ credenciales, onCerrar }) => {
  ...
  return (
    <Modal abierto={!!credenciales} onCerrar={onCerrar} titulo="✅ Bacteriólogo creado exitosamente">
      <div className="space-y-4">
        <p className="text-sm text-gray-600">Comparte estas credenciales con el bacteriólogo encargado.</p>
        <div className="bg-green-50 rounded-xl p-4 space-y-3">
          <div>
            <p className="text-xs text-gray-500 mb-1">Bacteriólogo</p>
...
const Bacteriologos = () => {
  ...
  const cargar = async () => {
    try {
      const { data } = await listarUsuariosApi({ rol: 'bacteriologo' });
      setBacteriologos(data.data);
    } catch {
      toast.error('Error al cargar bacteriólogos');
    } finally {
      setCargando(false);
    }
  };
  ...
  const onSubmit = async ({ documento: _doc, ...datos }) => {
    setGuardando(true);
    try {
      const { data } = await crearUsuarioApi({ ...datos, rol: 'bacteriologo' });
      setBacteriologos((prev) => [data.data, ...prev]);
      cerrarModal();
      ...
      toast.error(err.response?.data?.mensaje || 'Error al crear bacteriólogo');
    } finally {
      setGuardando(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Bacteriólogos encargados</h2>
          <p className="text-gray-500 text-sm mt-1">Gestión de bacteriólogos y sus credenciales de acceso</p>
        </div>
        <Button onClick={() => setModalAbierto(true)}>+ Nuevo bacteriólogo</Button>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        {cargando ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="mt-3 text-sm font-medium text-gray-500">No hay bacteriólogos registrados</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-gray-600">Bacteriólogo</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {bacteriologos.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-gray-800">{b.nombre} {b.apellido}</td>
                  <td className="px-4 py-3 text-gray-500">{b.email}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs">
                    {b.ultimoAcceso
                      ? new Date(b.ultimoAcceso).toLocaleDateString('es-CO')
                      : 'Nunca'}
                  </td>
                  <td className="px-4 py-3">
                    <span className={b.activo ? 'badge badge-green' : 'badge badge-gray'}>
      ...
      <Modal abierto={modalAbierto} onCerrar={cerrarModal} titulo="Registrar nuevo bacteriólogo">
